...
def main():
    ...
    freeze_prefixes = tuple(cfg.get('freeze_module_prefixes', []))
    if freeze_prefixes:
        frozen_params = 0
        frozen_names = []
        ...
        for module_name in cfg.get('freeze_module_eval', []):
            module = model
            for attr in module_name.split('.'):
                module = getattr(module, attr, None)
                if module is None:
            ...
            if module is not None:
                module.eval()
        logger.info(f'Frozen module prefixes: {freeze_prefixes}')
        logger.info(f'Frozen parameters: {frozen_params / 1000 / 1000:.3f} M')
        logger.info(f'Frozen parameter examples: {frozen_names}')

    if cfg.get('lora_config', None):
        from adzoo.drivetransformer.mmdet3d_plugin.models.utils.lora import apply_lora
        lora_stats = apply_lora(model, cfg.lora_config)
        logger.info(f"Applied LoRA: {lora_stats['num_replaced']} Linear modules; "
                    f"trainable={lora_stats['trainable_params'] / 1000 / 1000:.3f}M / "
                    f"total={lora_stats['total_params'] / 1000 / 1000:.3f}M")
        logger.info(f"LoRA module examples: {lora_stats['replaced'][:30]}")
    ...
    if cfg.get('freeze_module_eval', []):
        runner.register_hook(FrozenModuleEvalHook(cfg.freeze_module_eval), priority='VERY_HIGH')
        logger.info(f'Keeping frozen modules in eval mode: {cfg.freeze_module_eval}')
    if distributed:
